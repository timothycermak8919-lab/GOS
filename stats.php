...
if (isset($_POST['resetStatValue'])) {
  if($char['respec']==1){
    $message = "Character skills reset!";
    $id = $char['id'];
    //we will have to set their points as well
    $sql = "UPDATE Users SET respec = 0 WHERE id = $id";
    mysqli_query($db, $sql);


    $skillsTemp = serialize(array(999));
    $active = serialize(array());
    $sql = "UPDATE Users_data SET skills = '$skillsTemp', active = '$active' WHERE id = $id";
    mysqli_query($db, $sql);
    
    $refund = $stats['skill_pts_used'] + $char['points'];

    $sql = "UPDATE Users_stats SET skill_pts_used = 0, highest_skill = 0, num_active_skills = 0 WHERE id = $id";
    mysqli_query($db, $sql);  

    $sql = "UPDATE Users SET points = $refund WHERE id = $id";
    mysqli_query($db, $sql);   
    
    $stats['skill_pts_used'] = 0;
    $stats['highest_skill'] = 0;
    $stats['num_active_skills'] = 0;
    $char['points'] = $refund;
    $char['skills'] = array(999);
    $char['active'] = array();
  }
}
...
$askills = $char['active'];
if (is_string($askills)) {
    $askills = unserialize($askills);
}
    ...
    <div class="col-sm-12">
      <div id="content">
        ...
        ?>  
         ...
         <li <?php if ($tab == -3) echo "class='active'";?>><a style="color:red;" href="#reset_tab" data-toggle="tab">Reset</a></li>
          <?php } ?>
        </ul>
        ...
        <div class="tab-content">
          ...
          <div class="tab-pane <?php if ($tab == -4) echo 'active';?>" id="reset_tab">
            <div class='row'>
              <p><b style="color:red;">Reset Stats</b></p>
              <p><i style="color:red;">You may only do this once!</i></p>
              </br>
              </br>
              <form class='form-inline' name='startformreset' action='stats.php' method='POST'>
              <input name="resetStatValue" id="resetStatValue" hidden></input>
              <input type="submit" value="Reset Character Stats" style="width:200px;" class="btn btn-danger btn-md btn-block"></input>
              </form>
