...
$noter=mysqli_real_escape_string($db,$_GET['note']);
$request=mysqli_real_escape_string($db,$_GET['request']);
$new_stance=intval(mysqli_real_escape_string($db,$_GET['stance']));
$id=$char['id'];
...
if ($name == $society_char['leader'] && $lastname == $society_char['leaderlast'] && $new_stance && $new_stance <= 2 && $new_stance >= -2 && $soc_name_char != $soc_name)
{
  ...
  if ($new_stance == -1 || $new_stance == 2 )
  {
    ...
    if ($stance[str_replace(" ","_",$soc_name)]==1 && $new_stance != 1)
    {
      ...
      $result = mysqli_query($db,"UPDATE Soc SET support='$support_str' WHERE name='$soc_name_char' ");
    
      $support2_str = serialize($support2);
      $result = mysqli_query($db,"UPDATE Soc SET support='$support2_str' WHERE name='$soc_name' ");
    }
    ...
    //$result = mysqli_query($db,"UPDATE Soc SET stance='$changed_stance' WHERE name='$soc_name_char' ");
  ...
  else if (($new_stance == 1 || $new_stance == -2))
  {
    ...
    if ($request && $reqnote['special'] == $soc_name)
    {
      ...
      $result = mysqli_query($db,"UPDATE Notes SET body='$reqnote[body]' WHERE id='$noter' ");
